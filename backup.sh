...
upload_to_bucket() {
    ...
    # Set expected size to 100GB (107374182400 bytes) to prevent multipart upload errors
    s3 cp - "s3://$S3_BUCKET_NAME/db_backups/$(date +%Y/%m/%d/backup-%H-%M-%S.dump)" --expected-size 107374182400
}
